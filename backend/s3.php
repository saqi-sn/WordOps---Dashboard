...
// Delete an object (SigV4 DELETE, empty body). $key is relative to the prefix.
function s3_delete_file(string $key): array {
    $bucket   = s3_cfg('bucket');
    $endpoint = s3_cfg('endpoint', 'https://s3.us-east-1.amazonaws.com');
    $region   = s3_cfg('region', 'us-east-1');
    $accessId = s3_cfg('key');
    ...
    $kSigning = hash_hmac('sha256', 'aws4_request', $kService, true);
    $signature = hash_hmac('sha256', $stringToSign, $kSigning);
    $authz = "AWS4-HMAC-SHA256 Credential=$accessId/$scope, SignedHeaders=$signedHeaders, Signature=$signature";
    ...
    // S3 DELETE returns 204 (also 200); treat 404 as already-gone = success.
    if (($code >= 200 && $code < 300) || $code === 404) return ['ok' => true];
    if ($code === 0) return ['ok' => false, 'error' => "S3 connection failed: $err"];
    return ['ok' => false, 'error' => "S3 $code: $resp"];
}
